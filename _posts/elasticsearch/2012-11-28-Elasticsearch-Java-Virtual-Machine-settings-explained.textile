...
If you operate with *very large heaps of more than 6-8 GB*, that is higher than the size the CMS garbage collector was designed for, and you encounter *unacceptable long stop-the-world pauses*, adjust the *CMSInitiatingOccupancyFraction* setting to reduce the probability of long GC runs, or try to *reduce the maximum heap* under the limit, or enable the *G1 garbage collector*.
