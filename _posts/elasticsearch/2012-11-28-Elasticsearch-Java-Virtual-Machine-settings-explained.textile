...
But it may happen, while monitoring the memory your Elasticsearch cluster nodes, you could get tempted by changing some of the JVM settings. Will it improve your situation or not? 

This blog tries to demystify the preconfigured Elasticsearch settings and discusses the most common adjustments.

h2. Overview of the Elasticsearch JVM settings

These are the preconfigured setting for Elasticearch 0.19.11
...
* overrides automatic heap memory setting of the JVM in favor of adjusting the *maximum default heap size* to a mere *1GB*
* assume that forced garbage collection runs at *75 percent allocation of your heap size* do not interfere with your workload requirements
* disable the *Java 7 default G1 collector*

h2. The memory structure of a JVM process

Before we can give some rules of thumb for Elasticsearch tuning, we discuss some concepts of the JVM, so we can explain why the settings in the Elasticsearch distribution are reasonable.

For now, we assume the Sun/Oracle HotSpot JVM. Elasticseach can run under JVMs of different vendors, but this is not the topic here.
...
The heap memory is an important size, because Java depends on a reasonably sized heap size, and the JVM can be instructed to grab only a limited amount of memory from the operating system during the JVM lifetime. 
...
The risk of a large heap is higher memory pressure. Note, if Java can operate with large heaps, this memory is no longer available to the rest of the system. If memory gets tight, operating systems tend to react with paging (swapping), and, in emergency situations, when all other methos of gaining memory back, even with process killing. If paging occurs, overall performance will degrade. and also garbage collection performance degrades. So, don't allocate too much memory for the heap.
...
Note, if using G1 garbage collector, it can be expected that memory within the maximum heap limit might be given back to the operating system. 
...
For Elasticsearch, the G1 means no more stop-the-world pauses and more flexible memory management, because buffer memory and system caches for input/output can better utilize the RAM resources of the machine. This comes at the price of less maximum performance because G1 utilizes more CPU.

h2. Strategies against performance degradation

First, make yourself sure about your performance strategy. Do you want maximum speed or maximum throughput?
...
* enable monitoring of your settings
...
You may try to *reduce the heap usage* in Elasticsearch indexing by adjusting the segment merging with the @index.merge.policy.segments_per_tier@ parameter.
...
To *enhance the CMS garbage collection*, you might add a reasonable -XX:CMSWaitDuration parameter.
...
fn2. It was reported to the author that Elasticsearch can index 50 million documents with the default heap setting before the performance degraded near to a halt.

fn3. "http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=7029167":http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=7029167 

fn4. For examples, see "http://www.datastax.com/dev/blog/whats-new-cassandra-066":http://www.datastax.com/dev/blog/whats-new-cassandra-066 and "https://issues.apache.org/jira/browse/CASSANDRA-1214":https://issues.apache.org/jira/browse/CASSANDRA-1214
